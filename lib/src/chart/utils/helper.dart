...
Paint _getLinearGradientPaint(
  ...
  gradientPaint = Paint()
    ...
    ..style = PaintingStyle.fill;
  debugPrint('gradientPaint $gradientPaint');
...
void renderStackedAreaSeries(
  ...
  _StackedSeriesBase<dynamic, dynamic> stackedArea100Series;
    ...
    if (point.isVisible) {
      ...
      if (series.emptyPointSettings.mode != EmptyPointMode.drop) {
        for (int j = pointIndex - 1; j >= startPoint; j--) {
          ...
          if (stackedArea100Series is StackedAreaSeries) {
            if (stackedArea100Series.borderDrawMode ==
                BorderDrawMode.excludeBottom)
              _strokePath.lineTo(point1.x, point2.y);
            else if (stackedArea100Series.borderDrawMode == BorderDrawMode.all)
              _strokePath.lineTo(point2.x, point2.y);
          } else if (stackedArea100Series is StackedArea100Series) {
            if (stackedArea100Series.borderDrawMode ==
                BorderDrawMode.excludeBottom)
              _strokePath.lineTo(point1.x, point2.y);
            else if (stackedArea100Series.borderDrawMode == BorderDrawMode.all)
  ...
  for (int j = series._dataPoints.length - 1; j >= startPoint; j--) {
    ...
    if (previousSeries.emptyPointSettings.mode != EmptyPointMode.drop ||
      ...
      _path.lineTo(point2.x, point2.y);
      if (stackedArea100Series is StackedAreaSeries) {
        if (stackedArea100Series.borderDrawMode == BorderDrawMode.excludeBottom)
          _strokePath.lineTo(point1.x, point2.y);
        else if (stackedArea100Series.borderDrawMode == BorderDrawMode.all)
          _strokePath.lineTo(point2.x, point2.y);
      } else if (stackedArea100Series is StackedArea100Series) {
...
void renderStackedLineSeries(_StackedSeriesBase<dynamic, dynamic> series,
...
void calculateControlPoints(XyDataSeries<dynamic, dynamic> series) {
  List<num> yCoef = <num>[];
  ...
  if (series is SplineAreaSeries && xValues.isNotEmpty) {
    ...
    for (int pointIndex = 0; pointIndex < xValues.length - 1; pointIndex++) {
      ...
      series.drawPoints = <_ControlPoints>[];
      if (xValues[pointIndex] != null &&
        ...
        if (splineType == SplineType.monotonic) {
          ...
          series.drawControlPoints.add(_ListControlPoints(series.drawPoints));
        } else if (splineType == SplineType.cardinal) {
